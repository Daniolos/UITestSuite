...
from config import ICONS_PATH, SCREENSHOTS_PATH
...
def check_loading_indicator(icon_key: str):
    for _ in range(100):
        screenshot_path = f"{SCREENSHOTS_PATH}/{icon_key}.png"
        pyautogui.screenshot(screenshot_path)
        pyautogui.sleep(0.2)
        if not locate_icon_center_on_image(screenshot_path, f"{ICONS_PATH}/{icon_key}.png"):
            return
...
def click_icon_on_screen(icon_key: str):
    screenshot_path = f"{SCREENSHOTS_PATH}/{icon_key}.png"
    pyautogui.screenshot(screenshot_path)
    x, y = locate_icon_center_on_image(screenshot_path, f"{ICONS_PATH}/{icon_key}.png")
    pyautogui.click(x, y)


def click_text_on_screen(text: str):
    image = pyautogui.screenshot(f"{SCREENSHOTS_PATH}/{text}.png")
